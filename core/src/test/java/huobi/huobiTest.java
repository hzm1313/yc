...
public class huobiTest {
    ...
    public static boolean isPalindrome(String s) {
        ...
        for (int i = 0; i < strArray.length; i++) {
            int charNum = strArray[i];
            if (charNum <= 90 && charNum >= 65) {
                newArray[max] = charNum + 32;
                max++;
            } else if (charNum >= 97 && charNum <= 122) {
                newArray[max] = charNum;
                max++;
...
//            kLineTypeList.add(Constants.HUOBI_BFT);
            String fileDire = "F:\\数据\\20180930";
            for (int klineTypeIndex = 0; klineTypeIndex < kLineTypeList.size(); klineTypeIndex++) {
                String dtName = kLineTypeList.get(klineTypeIndex) + "usdt";//Constants.HUOBI_BTCUSDT;
                dtName = dtName.trim();
...
//                timeTypeList.add(Constants.HUOBI_60min);
                for (int timeIndex = 0; timeIndex < timeTypeList.size(); timeIndex++) {
                    ...
                    String startTimeStr = "2017/10/01 00:00:00";//"2017/10/27 00:00:00";
